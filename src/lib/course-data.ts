...
export interface CourseModule {
  ...
  published?: boolean; // true = active in agent portal, false = draft in admin console
  description: string;
  duration?: string;
...
export function getCourseModules(): CourseModule[] {
  ...
  try {
    ...
    // Ensure published defaults to true for older entries
...
/**
 * Persist updated course modules locally & emit sync event
 */
...
/**
 * Save / Update a module in Firestore & Local Storage
 */
...
/**
 * Delete a module from Firestore & Local Storage
 */
...
/**
 * Sync Firestore collection "agent_resources" in real-time
 */
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        if (!snap.empty) {
          ...
          // If Firestore collection is empty, seed initial modules to Firestore
...
/**
 * Track an engagement event on a module (view, play_video, share_tour, download)
 */

...
export function ManagerCollectionBalancing() {
  ...
  const { members, currentStoreId, stores, role } = useRole();
  ...
  const handleConfirmCreateCollection = async () => {
    ...
    await createCollection({
      ...
      storeName: currentStoreObj?.name || "Main Store",
      items: itemsPayload,
  ...
  const handleConfirmBalanceUp = async () => {
    ...
    await balanceUpCollection(balanceTarget.id, {
      itemsBalancing,
      cashRemittedNgn,
      balancedBy: profile?.name || user?.email || "Store Admin",

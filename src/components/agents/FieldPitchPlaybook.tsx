...
export function FieldPitchPlaybook({ referralCode = "AGENT" }: FieldPitchPlaybookProps) {
  ...
  // ROI Pitch Calculator State
  const [dailyTurnover, setDailyTurnover] = useState<number>(85000);
  const [shrinkageRate, setShrinkageRate] = useState<number>(4); // 4% average theft/error rate
  const [copiedPitch, setCopiedPitch] = useState<boolean>(false);

  // Calculations
  const dailyLoss = (dailyTurnover * shrinkageRate) / 100;
  const monthlyLoss = dailyLoss * 26; // 26 working days per month
  const yearlyLoss = monthlyLoss * 12;
  const nexaProCostYearly = 36000; // ₦3,000 * 12
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        {/* LEFT: OBJECTION PLAYBOOK (7 COLS) */}
        ...
        {/* RIGHT: INTERACTIVE MERCHANT ROI CALCULATOR (5 COLS) */}
